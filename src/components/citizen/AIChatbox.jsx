...
export const AIChatbox = ({ onOpenMyReports }) => {
  ...
  const handleSaveApiKey = () => {
    ...
    addToast({
      ...
      message: key ? "Gemini AI live model activated for conversational reporting!" : "Gemini API key removed. Using offline emergency engine."
    });
  };
    ...
    <>
      ...
      {isOpen && (
        ...
        >
          ...
          {/* Optional API Key Setup Notice / Modal */}
          {apiKeyModalOpen && (
            <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-xs z-30 p-4 flex items-center justify-center animate-in fade-in duration-150">
              <div className="bg-white rounded-2xl p-5 shadow-2xl border border-slate-200 w-full max-w-sm space-y-3.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-slate-800">
                    <Key className="w-4 h-4 text-blue-600" />
                    <h4 className="font-bold text-sm">Gemini AI API Key</h4>
                  </div>
                  <button
                ...
                <p className="text-xs text-slate-600 leading-relaxed">
                  Provide your Google Gemini API key to unlock natural disaster intake, voice translation, and high-accuracy structured report generation.
                </p>
                ...
                <div className="pt-2 flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-2.5 py-1.5 text-xs text-slate-500 hover:text-slate-700 font-medium"
                    >
                      Clear
                    </button>
                    <button
          ...
          {/* Quick API Key Reminder Pill (if not configured yet) */}
          {!hasApiKey && (
            <div className="bg-amber-50 border-b border-amber-200 px-3 py-1.5 text-[11px] text-amber-800 flex items-center justify-between">
              <span className="flex items-center gap-1 truncate">
                ...
                <span>Running offline rule engine. Add Gemini Key for full AI.</span>
              </span>
              <button
                onClick={() => setApiKeyModalOpen(true)}
